...
class TestProjectedDonorAcceptorRatio(unittest.TestCase):
    ...
    def test_pmj_one(self):
        ...
        result = projected_donor_acceptor_ratio(acceptor_coords, hydrogen_coords, donor_coords)
        expected = 0.50
...
class TestNormalizationFactor(unittest.TestCase):
    ...
    def test_norm_factor_one(self):
        donor_coords = np.asarray([3, -1, -7])
        hydrogen_coords = (np.asarray([1000, 1000, 1000]), np.asarray([2, 2, 2]), np.asarray([2, -4, 7]))
        acceptor_coords = (np.asarray([5, -2, -9]), np.asarray([3, 3, 3]), np.asarray([1, 1, 1]))
    ...
    def test_norm_factor_two(self):
        donor_coords = np.asarray([3, -1, -7])
        hydrogen_coords = (np.asarray([10, 2, -3]), np.asarray([2, 2, 2]), np.asarray([2, -4, 7]))
        acceptor_coords = (np.asarray([5, -2, -9]), np.asarray([3, 3, 3]), np.asarray([1, 1, 1]))
        result = normalization_factor(acceptor_coords, hydrogen_coords, donor_coords)
